...
export async function POST(request: NextRequest) {
  try {
    const { imageUrl, amount = 2, feature } = await request.json();
    ...
    // Deduct credits
    const creditCost = 1; // Sharpen costs 1 credit
    const creditDeducted = await deductCredits(user.id, creditCost, "sharpen", "Applied sharpen effect");
    ...
    const processedBuffer = await sharpenImage(buffer, amount);
    const metadata = await getImageMetadata(processedBuffer);

    const timestamp = Date.now();
    const filename = `${timestamp}-sharpened.png`;
