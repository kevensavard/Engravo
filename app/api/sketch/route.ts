...
export async function POST(request: NextRequest) {
  try {
    ...
    // Deduct credits
    const creditCost = 2; // Sketch costs 2 credits
    const creditDeducted = await deductCredits(user.id, creditCost, "sketch", "Applied sketch effect");
    ...
    const processedBuffer = await sketch(buffer);
    const metadata = await getImageMetadata(processedBuffer);

    const timestamp = Date.now();
    const filename = `${timestamp}-sketch.png`;
    ...
    return NextResponse.json(
      { error: "Failed to apply sketch effect" },
      { status: 500 }
    );
  }
}
